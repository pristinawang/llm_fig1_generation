...
def save_ids_latex(target_dict, latex_files_path, backup_dir_path, backup_db_file_name='arxiv_id_db_backup.csv'):
...
def regular_full_extraction(target_dict, latex_files_path, benchmark_dir_path, backup_dir_path, cpu_num, backup_db_file_name='arxiv_id_db_backup.csv'):
    for year, venue in target_dict.items():
        csv_path = os.path.join(benchmark_dir_path, venue, year, f'{venue}_{year}.csv')
        backup_db_path = os.path.join(backup_dir_path, backup_db_file_name)
